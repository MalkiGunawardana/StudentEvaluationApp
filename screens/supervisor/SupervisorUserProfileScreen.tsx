...
import SupervisorBottomNavBarP2 from '../../components/SupervisorBottomNavBarP2';
...
export default function SupervisorUserProfileScreen({ navigation }: any) {
  const authContext = useAuth();
  const route = useRoute();
  const isWeb = Platform.OS === 'web';

  const [currentPerformance, setCurrentPerformance] = useState<"performance1" | "performance2">("performance1"); // Default to performance1
  ...
  if (!authContext || !authContext.auth) {
    useEffect(() => {
      console.error("SupervisorUserProfileScreen: AuthContext or authContext.auth is not available.");
    }, []);
    return (
  ...
  const { auth, setAuthData } = authContext;
  const { idToken, uid } = auth;

  useEffect(() => {
    // Read the 'performance' param from the route. Default to 'performance1' if not provided.
    const performanceContext = (route.params as { performance?: 'performance1' | 'performance2' })?.performance || 'performance1';
    setCurrentPerformance(performanceContext);
  }, [route.params]);
  ...
  const handleSaveName = async () => {
    ...
    setSavingName(true);
    try {
      await updateAuthUserProfile(idToken, { displayName: `${newFirstName.trim()} ${newLastName.trim()}` }); // Corrected method name
      await updateUserName(uid, newFirstName.trim(), newLastName.trim(), idToken);
      
      // Update AuthContext with new name
      if (auth) { // Ensure auth object exists
        await setAuthData({
          idToken: auth.idToken,
          uid: auth.uid,
          role: auth.role,
          email: auth.email, // Explicitly pass email
          // firstName and lastName are being updated here
  ...
  const confirmLogout = async () => {
    ...
    try {
      if (typeof authContext.clearAuthData !== 'function') {
        displayNotification("Logout service is currently unavailable. Please try again later.", "error", "Logout Error", { autoClose: false });
        console.error("SupervisorUserProfileScreen: authContext.clearAuthData method is not a function.", authContext);
        return;
      }
      await authContext.clearAuthData();
    } catch (error: any) {
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        ...
        <Button
          ...
          labelStyle={styles.performanceButtonLabel}
          icon="ballot-outline"
        >
          Select Performance
        </Button>
      ...
      {currentPerformance === "performance1" ? (
