...
import { ActivityIndicator, FlatList, Image, Modal, SafeAreaView, ScrollView, StyleSheet, TouchableOpacity, View } from "react-native"; // Modal for results, not notifications
...
import { calculateRoundScore, isRoundDataEmpty } from "../../utils/calculationUtils"; // prettier-ignore
...
// For web download
function downloadCsvWeb(filename: string, csvString: string) {
...
export default function Performance1ResultsScreen({ navigation }: any) {
  const { auth } = useAuth();
  ...
  const [downloadingMaleCsv, setDownloadingMaleCsv] = useState(false); // For Male P1 results
  const [downloadingFemaleCsv, setDownloadingFemaleCsv] = useState(false); // For Female P1 results
  ...
  const handleClearSearch = () => {
    setSearchTerm("");
    setFilteredStudents([]); // Clear the displayed results
  };
  const handleSearch = async () => {
    if (!searchTerm.trim()) {
      displayModal("Please enter a name or ID to search.", "error", "Search Error");
      return;
    }
    if (!idToken) {
        ...
        for (const event of allEventDetails) {
          ...
          if (marks) {
            ...
            studentEventResults.push({
              marks,
              eventDetails: event,
              calculatedScores: { round1Score, round2Score, finalScore }
            });
          }
        }
        if (studentEventResults.length > 0) {
          studentsWithResults.push({ ...student, eventResults: studentEventResults });
        }
      }
  ...
  //       if (!idToken) {
  //           displayModal("Authentication error.", "error", "Auth Error", { autoClose: false });
  //           setDownloadingAll(false); // Ensure loading state is reset
  //           return;
  //       }

  //       // fetchStudents should return students with the 'team' property
  //       const allStudentsList = await fetchStudents(idToken); 
  ...
  //       let csvString = "Student Name,Index No,Province,Team,Event Name,Event Gender,Performance,Round 1 Score,Round 2 Score,Final Score\n";
  //       let resultsFoundForAnyStudent = false;

  //       for (const student of allStudentsList) { // student object here should have 'team'
  //           for (const event of allEventDetails) {
  //               const marks = await fetchStudentMarksForEvent(student.id, event.id, idToken);
  //               if (marks) { // For "Download All", we might include all performance types or filter if needed
  ...
  //       const filename = "all_student_performance_results.csv";
  //       const fileUri = FileSystem.cacheDirectory + filename;
  //       await FileSystem.writeAsStringAsync(fileUri, csvString, { encoding: FileSystem.EncodingType.UTF8 });

  //       if (await Sharing.isAvailableAsync()) {
  //           await Sharing.shareAsync(fileUri, { mimeType: 'text/csv', dialogTitle: 'Download All Results', UTI: 'public.comma-separated-values-text' });
  //       } else {
  ...
  const handleDownloadGenderSpecificP1ResultsCsv = async (genderCategory: 'Male' | 'Female') => {
    if (!idToken) {
      displayModal("Authentication error.", 'error', "Auth Error", { autoClose: false });
      return;
    }
    if (allEventDetails.length === 0) {
      displayModal(`No events available to generate ${genderCategory.toLowerCase()} P1 results.`, 'info', "No Events");
      return;
    }

    if (genderCategory === 'Male') setDownloadingMaleCsv(true);
    else setDownloadingFemaleCsv(true);
    displayModal(`Preparing ${genderCategory} Performance 1 CSV...`, 'info', "CSV Export", { autoClose: true, autoCloseDelay: 4000 });
    ...
    try {
      ...
      let csvString = `Performance 1 Results\n`;
      csvString += `Gender:,${genderCategory}\n\n`;
      csvString += "ID,Student Name,Province,Team,Event Name,Round 1 Score,Round 2 Score,Final Score\n";
      ...
      for (const event of genderSpecificEvents) {
        for (const student of allStudentsList) {
          ...
          if (marks && marks.performance === "performance 1") {
            ...
            const studentNameCsv = safeCsvText(student.fullName);
            const province = safeCsvText(student.province);
            const team = safeCsvText(student.team);
            ...
            csvString += `${indexNo},${studentNameCsv},${province},${team},${safeCsvText(event.eventName)},${round1ScoreStr},${round2ScoreStr},${finalScoreStr}\n`;
          }
        }
      }
      ...
      if (!resultsFoundForGender) {
        hideFeedbackModal();
        displayModal(`No 'Performance 1' marks found for ${genderCategory.toLowerCase()} events.`, 'info', "No Data");
        if (genderCategory === 'Male') setDownloadingMaleCsv(false); else setDownloadingFemaleCsv(false);
        return;
      }

      const filename = `${genderCategory.toLowerCase()}_performance_1_results.csv`;
      if (isWeb) {
        downloadCsvWeb(filename, csvString);
        hideFeedbackModal();
        displayModal(`${genderCategory} P1 Results CSV downloaded.`, 'success', "Downloaded");
      } else {
        const fileUri = FileSystem.cacheDirectory + filename;
        await FileSystem.writeAsStringAsync(fileUri, csvString, { encoding: FileSystem.EncodingType.UTF8 });
        hideFeedbackModal();
        if (await Sharing.isAvailableAsync()) {
          await Sharing.shareAsync(fileUri, { mimeType: 'text/csv', dialogTitle: `Download ${genderCategory} P1 Results`, UTI: 'public.comma-separated-values-text' });
          displayModal(`${genderCategory} P1 Results CSV shared.`, 'success', "Shared");
        } else {
          displayModal("Sharing is not available on this device.", "error", "Device Error");
        }
      }
      ...
      hideFeedbackModal();
      displayModal(`Failed to generate ${genderCategory} P1 CSV: ${error.message}`, 'error', "CSV Error", { autoClose: false });
    } finally {
      if (genderCategory === 'Male') setDownloadingMaleCsv(false); else setDownloadingFemaleCsv(false);
    }
  };
  ...
  const TopBarContent = () => (
    <View style={styles.topBarInternal}>
      <View style={{flex: 1}}>
        <Text style={styles.pageTitle}>Performance 1 Results</Text>
        ...
        <View style={{ flexDirection: 'row', height: '100%', backgroundColor: '#f8f9fa' }}>
          ...
          <View style={{ flex: 1, padding: 32 }}>
            <View style={{ marginBottom: 32 }}>
              <Text style={{ fontSize: 32, fontWeight: 'bold', color: '#1A202C' }}>Performance 1 Results</Text>
            ...
            <View style={{ backgroundColor: '#fff', padding: 24, borderRadius: 12, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 4, marginBottom: 24 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
                <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#bbb', borderRadius: 8, backgroundColor: '#fff', height: 48, paddingLeft: 8, marginRight: 8 }}>
                  ...
                  />
                  {searchTerm ? (
                    <IconButton icon="close-circle-outline" size={20} onPress={handleClearSearch} style={{ marginRight: 0, marginLeft: -4 }} iconColor="#e53935" />
                  ) : null}
                </View>
                <TouchableOpacity
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginVertical: 8, marginBottom: 16 }}>
                <Button
                  icon="download"
                  mode="contained"
                  onPress={() => handleDownloadGenderSpecificP1ResultsCsv('Male')}
                ...
                <Button
                  icon="download"
                  mode="outlined"
                  onPress={() => handleDownloadGenderSpecificP1ResultsCsv('Female')}
                  disabled={downloadingMaleCsv || downloadingFemaleCsv || loading || loadingInitialData}
        ...
        <View style={styles.filterCard}>
          <View style={styles.searchRow}>
            <View style={{flex: 1, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#bbb', borderRadius: 8, backgroundColor: '#fff'}}>
              ...
              />
              {searchTerm ? (
                <IconButton icon="close-circle-outline" size={20} onPress={handleClearSearch} style={styles.clearSearchIconBtn} iconColor="#e53935"/>
              ) : null}
            </View>
            <IconButton 
              icon="magnify" 
            ...
            renderItem={({ item }) => (
              <TouchableOpacity style={styles.studentItem} onPress={() => openResultModal(item)}>
                <Text style={styles.studentName}>
                  {`${safeRenderText(item.fullName, 'Unknown Name')} (${safeRenderText(item.indexNo, 'No ID')})`}
                </Text>
              </TouchableOpacity>
            )}
            ListEmptyComponent={
                    ...
                    selectedStudentForModal.eventResults.map((result, index) => (
                      <View key={index} style={styles.eventResultBlock}>
                        <Text style={styles.modalEventName}>
                          <Text style={styles.modalLabel}>Event: </Text>{result.eventDetails.eventName}
                        </Text>
                        <Text style={styles.finalScoreText}>
                          <Text style={styles.modalLabel}>Final Score: </Text>{formatScoreForDisplay(result.calculatedScores.finalScore)}
                        </Text>
                      </View>
                    ))
                  ) : (
